...
def run_tests(zone_id):
    ...
    test_reaches(zone_id, 'n_reaches')
    test_members(zone_id, 'n_members')
    test_eperms(zone_id, 'n_eperms')
...
def test_reaches(cmd, zone_id):
    ...
    assert_true(query(zone_id, cmd, src='luke', dst='krakow')['result'])
    assert_false(query(zone_id, cmd, src='anne', dst='lisbon')['result'])
    assert_true(query(zone_id, cmd, src='luke', dst='dhub_members')['result'])


def test_members(zone_id, cmd):
    assert_equal(set(query(zone_id, cmd, of='admins')['result']),
                 {'luke'})
    assert_equal(set(query(zone_id, cmd, of='eo_data')['result']),
                 {'luke', 'bob', 'alice'})


def test_eperms(zone_id, cmd):
    assert_equal(query(zone_id, cmd, src='alice', dst='bob')['result'],
                 None)
    assert_equal(query(zone_id, cmd, src='alice', dst='ebi')['result'],
                 '11000')
    assert_equal(query(zone_id, cmd, src='audit', dst='cyfnet')['result'],
                 '11001')
    assert_equal(query(zone_id, cmd, src='audit', dst='eosc')['result'],
                 '11001')
    assert_equal(query(zone_id, cmd, src='tom', dst='primage')['result'],
                 '11011')
