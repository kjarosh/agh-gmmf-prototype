...
import com.github.kjarosh.agh.pp.util.ModelObjectMapper;
...
public class InMemoryGraph implements Graph {
    ...
    public InMemoryGraph(ZoneId currentZoneId) {
        this.currentZoneId = currentZoneId;
    }

    @SneakyThrows
    public static InMemoryGraph deserialize(InputStream serialized, ZoneId currentZoneId) {
        Json value = ModelObjectMapper.INSTANCE.readValue(serialized, Json.class);
        InMemoryGraph graph = new InMemoryGraph(currentZoneId);
    ...
    public Vertex getVertex(VertexId id) {
        ...
        if (vertex == null && !hasVertex(id)) {
            throw new RuntimeException(
                    "Vertex " + id + " not found in zone " + currentZoneId);
        }
        return vertex;
    }
    ...
    public void serialize(OutputStream os) {
        ...
        ModelObjectMapper.INSTANCE.writeValue(os, json);
    }
